...
void replay_SBSGEM( int runnum=1219, int firstsegment=0, int maxsegments=1, const char *fname_prefix="eel125", long firstevent=0, long nevents=2000, int pedestalmode=0 ){
  
  gSystem->Load("/home/sbs-onl/sbs/build/libsbs.so");
  ...
  // SBSGenericDetector* sbstrig= new SBSGenericDetector("sbstrig","BigBite shower ADC trig");
  // sbstrig->SetModeADC(SBSModeADC::kADC);
  // sbstrig->SetModeTDC(SBSModeTDC::kTDC);
  // sbstrig->SetStoreEmptyElements(kFALSE);
  // sbs->AddDetector( sbstrig );
  ...
  */
  ...
  //   if( ftest == "sbsgem" || ftest == "e1209019_trigtest" ){
  //     codafilename.Form("%s_%d.evio.0.%d", fname_prefix, runnum, 0 );
  //   }
  ...
  while( segcounter < max1 && segment - firstsegment < maxsegments ){
    ...
    TString codafilename;
    //codafilename.Form( "%s/eel125_%d.evio.%d", prefix.Data(), runnum, segment );
    codafilename.Form("%s_%d.evio.%d.%d", fname_prefix, runnum, stream, segment );

    TString ftest(fname_prefix);
    if( ftest == "e1209019" || ftest == "e1209019_trigtest" ){
  ...
  TString outfilename;
  outfilename.Form( "%s/e1209019_replayed_%d_stream%d_seg%d_%d.root", prefix.Data(), runnum,
  ...
  TString odef_filename = "replay_sbsgem_trigtdc_shower.odef";
